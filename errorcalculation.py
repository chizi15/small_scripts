...
def mae(actuallist, predictlist):
    ...
    mae = np.mean(abs(predict_array - actual_array))
...
def mape(actuallist, predictlist):
    '''
        np.mean absolute percentage error
        平均绝对误差率（相对百分误差绝对值的平均值）
    '''
    # actual_array = array(actuallist)
    # predict_array = array(predictlist)

    # mape = np.mean(abs(predict_array - actual_array) * 100 / actual_array)
    ...
    if len(predictlist) == 0: # 刚开始没有历史预测值
        ...
        # mape = np.mean([abs(m - n) * 100 / n for m, n in zip(predictlist, actuallist)])
...
def ols(actuallist, predictlist):
    ...
    distance = 0.5 * np.sum((actualarray - predictarray) ** 2)
    return distance


def rmse(actuallist, predictlist):
    '''
        root np.mean squared error
        均方根误差=标准误差
    '''
    ...
    rmse = sqrt(np.mean((predict_array - actual_array) ** 2))
...
def rae(actuallist, predictlist):
    ...
    rae = np.sum(abs(predict_array - actual_array)) / np.sum(abs(actual_array - np.mean(actual_array)))
...
def rrse(actuallist, predictlist):
    ...
    rrse = sqrt(np.sum((predict_array - actual_array) ** 2) / np.sum((actual_array - np.mean(actual_array)) ** 2))
...
def aic(actuallist, predictlist, k):
    ...
    rss = np.sum((predict_array - actual_array) ** 2)  # 残差平方和
