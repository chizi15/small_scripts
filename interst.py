...
print('-----------------------------------------------------------------------------------------------')
print('每期偿还等额资金（本金+手续费）: 1.本金和手续费相等；2.手续费递减，本金递增，总量相等', '\n')

print('交通分期，每期偿还等额资金（本金+手续费），本金和手续费相等')
num = [3, 6, 9, 12, 18, 24]
...
print('交通银行惠民贷，每期偿还等额资金（本金+手续费），手续费递减，本金递增，总量相等（可等价为本金和手续费相等的模式）')
num = [6, 12, 24]
handling_charge_total = [513.45, 1162.20, 2507.39]
handling_charge_revised = np.array(handling_charge_total) / np.array(num)
capital = 30000
handling_charge_rate = []
dr = []
ar = []
if len(num) == len(handling_charge_total):
    for item in num:
        # 属于每期等额偿还本金及手续费的情况，将(每期)分期手续费转化为实际(每期)分期手续费率(%)
        index = num.index(item)
        handling_charge_rate.append(hc2hcr(handling_charge=handling_charge_revised[index], capital=capital, n=item))
    for hcr in handling_charge_rate:
        # 将(每期)分期手续费率(%)转化为日利率(‱)及年利率(%)
        dr.append(hcr2dr(hcr))
        ar.append(hcr2ar(hcr))
...
print('招商分期，每期偿还等额资金（本金+手续费），本金和手续费相等')
...
print('广发分期，每期偿还等额资金（本金+手续费），本金和手续费相等')
num = 12
...
print('京东白条，还款时分期，每期偿还等额资金（本金+手续费），本金和手续费相等')
num = [3, 6, 12, 24]
